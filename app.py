...
try:
    ...
    st.error("❌ drug_data.xlsx not found or incorrect format.")
    st.stop()
...
payer = st.selectbox("Select Insurance (Payer)", df["Payer"].unique())
...
if st.button("Calculate"):
    ...
    for entry in drug_entries:
        ...
        drug_df = df[(df["Drug_Name"] == entry["drug"]) & (df["Payer"] == payer)]

        if drug_df.empty:
            st.error(f"No data for {entry['drug']} with payer {payer}")
            st.stop()
        ...
        allowable_per_unit = drug_data["Allowable_per_Unit"]

        units_billed = entry["dose"] / billing_unit
        ...
        detailed_rows.append({
            "Drug": entry["drug"],
            "Dose": entry["dose"],
            "Units Billed": round(units_billed, 2),
